...
while [ $# -gt 0 ]; do
  case "$1" in
    --key)
      [ -n "${2:-}" ] && [ -n "$(host_addr "$2")" ] || {
        echo "--key wymaga hosta: $HOST_LIST (hp jest zabronione)"; exit 2;
      }
      KEYHOST="$2"; shift 2 ;;
...
if command -v java >/dev/null; then
  ...
  [ "$JV" = "21" ] && ok "JDK $JV" || bad "JDK $JV - wymagany JDK 21"
...
if [ "$FAIL" != "0" ] && { [ -n "$KEYHOST" ] || [ "$DOBUILD" = "1" ]; }; then
  exit 1
fi
...
if [ -f "$JKS" ]; then
  ...
  # Only validated off-machine backup hosts can reach this point; never hp.
  ...
  [ -n "$RESTORED" ] && [ -f "$RESTORED" ] || { bad "brak jednoznacznego klucza w archiwum"; exit 1; }
  cp "$RESTORED" "$JKS"
  chmod 600 "$JKS"
  ...
  bad "brak klucza; uruchom z --key hetz|host|ubu|rpi4|rpi5"
fi
...
if [ -f "$KP" ]; then
  ...
  for property in storePassword keyPassword; do
    tr -d '\r' < "$KP" | grep -Eq "^$property=.+" || bad "puste $property - uzupelnij lokalnie z menedzera hasel"
  done
  tr -d '\r' < "$KP" | grep -qx "keyAlias=$ALIAS" || bad "keyAlias musi byc $ALIAS"
elif [ -n "$KEYHOST" ]; then
  ...
  bad "brak keystore.properties; --key tworzy szkielet, tryb sprawdzania niczego nie zapisuje"
fi
...
if [ "$DOBUILD" = "1" ] && [ "$FAIL" = "0" ]; then
  ...
  CERTS=$("$SIGNER" verify --print-certs "$APK") || { bad "nieprawidlowy podpis APK"; exit 1; }
  APK_FP=$(printf '%s\n' "$CERTS" | sed -n 's/^Signer #1 certificate SHA-256 digest: //p')
  [ "$APK_FP" = "$CERT_FP" ] || { bad "ODCISK APK SIE NIE ZGADZA: $APK_FP"; exit 1; }
  ok "podpis APK i odcisk certyfikatu zgodne"
  GOT=$(sha "$APK")
